...
```
git clone https://github.com/citw2/demo-glm
cd demo-glm
npm install
node demo.mjs
```

## Use a real GLM answer (BYOK)

With no key set, GLM answers in a deterministic **offline mock**, so the demo runs end to end with zero setup. Set your key to get a real answer:
...
The call is GLM’s standard **OpenAI-compatible** request (`https://open.bigmodel.cn/api/paas/v4/chat/completions`, model `glm-4.5-flash`). Your key is read from the environment and sent only to GLM’s own API. Override the base URL or model id without editing code via `SAIHM_GLM_URL` / `SAIHM_GLM_MODEL`.

## Why this matters

A per-vendor “memory” feature keeps your context inside one vendor’s account. SAIHM gives you memory that is:

1. **Yours / portable.** One live store grounds GLM *and* every other model (Claude, DeepSeek, Qwen, Kimi, GLM, GPT, a local model, your own agent) — no per-vendor export, no lossy one-time import.
